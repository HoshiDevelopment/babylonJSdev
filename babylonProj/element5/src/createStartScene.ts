...
function createHouses(scene: Scene, style: number) {  
    ...
    if (style == 3) {
      ...
      // houses are cloned instances of first two
...
function createTrees(scene: Scene) {
  ...
  function createHemisphericLight(scene: Scene) {
    const light = new HemisphericLight(
      "light",
      new Vector3(2, 1, 0), // change x value to move shadowa
      scene
    );
    light.intensity = 0.8;
...
function createArcRotateCamera(scene: Scene) {
  ...
  function importMeshA(scene: Scene, x: number, y: number) {
    ...
    item.then((result) => {
      let aplane: AbstractMesh = result!.meshes[0];
      aplane.position.x = x;
      aplane.position.y = y + 0.1;
      aplane.scaling = new Vector3(1, 1, 1);
      aplane.rotation = new Vector3(0, 1.5, 0);
    });
    return item;
  }
...
export default function createStartScene(engine: Engine) {
  ...
  let plane   = importMeshA(scene, 0, 0);
